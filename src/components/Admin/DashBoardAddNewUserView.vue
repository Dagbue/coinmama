...
export default {
  ...
  setup() {
    ...
    const profits = ref(0)
    const referral = ref('')
    const statusDeposit = ref('approved')
    const statusWithdrawal = ref('approved')
    const url = ref(null)
    const file = ref(null)
    ...
    const handleSubmit = async () => {
      try {
        ...
        await setDoc(doc(db, auth.currentUser.email, "USER"), {
          ...
          statusWithdrawal: statusWithdrawal.value,
          statusDeposit: statusDeposit.value,
    ...
    return {
      ...
      profits,file,statusDeposit,statusWithdrawal,
      handleSubmit, error,
